...
function send_subscriptions(ip: addr, peer_name: string)
	...
	for ( i in subscriptions )
		...
		# Subscribe.
		#
		# TODO: Change to require only one subscription.
		local init_dump = ev$init_dump;
		local ev_name = split_string(fmt("%s", ev$ev), /\n/)[0];
		local host_topic = fmt("/bro/osquery/host/%s", peer_name);

		log_peer("info", peer_name,
			 fmt("subscribing to event %s() with '%s'", ev_name, ev$query));
		...
		local ev_args1 = BrokerComm::event_args(host_subscribe, ev_name, ev$query, update_type, init_dump);
		BrokerComm::event(host_topic, ev_args1);
		init_dump = F;
		}

	local ev_args2 = BrokerComm::event_args(host_subscribe_end);
